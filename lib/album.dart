import 'dart:convert';
import 'package:araiband/detailalbum.dart';
...
class _AlbumLaguState extends State<AlbumLagu> {
  ...
  Future<List> _getDataAlbum() async {
    final response = await http.get("https://arai-band.000webhostapp.com/getdataalbum.php");
          ...
          child: GestureDetector(
            onTap: (){
              Navigator.push(context, 
                MaterialPageRoute(
                  builder: (context) => DetailAlbum(idAlbum: list[i]["id_album"], namaAlbum: list[i]["judul_album"], picAlbum: list[i]["foto_album"],)
                )
              );
            },
            child: Card(
                  ...
                  decoration: BoxDecoration(
                    ...
                    image: new DecorationImage(
                      ...
                      image: new NetworkImage(
                        "http://192.168.100.148/araiband/bahanarai/${list[i]['foto_album']}"
                      )
                    )
                  )
                ),
